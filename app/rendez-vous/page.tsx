...
export default function RendezVousPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error: e2 } = await supabase.from("pastoral_appointments").insert({
      user_id: user.id,
    ...
    return (
      <div style={{ background: "var(--page-bg)", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: 24, fontFamily: "var(--font-body)" }}>
        <div style={{ textAlign: "center", maxWidth: 400 }}>
          ...
          <h2 style={{ margin: "0 0 10px", fontSize: 22, fontWeight: 800, color: "var(--text-primary)", fontFamily: "var(--font-title)" }}>
            Demande envoyee !
          </h2>
          <p style={{ margin: "0 0 8px", color: "var(--text-secondary)", fontSize: 14, lineHeight: 1.6 }}>
            ...
            <strong>{new Date(form.preferred_date).toLocaleDateString("fr-FR")}</strong>{" "}
            a <strong>{form.preferred_time}</strong> a bien ete recue.
          </p>
          <p style={{ margin: "0 0 24px", color: "var(--text-muted)", fontSize: 13 }}>
            Le pasteur vous contactera pour confirmer le creneau. Que Dieu vous benisse !
          </p>
          <button
            onClick={() => { setSent(false); setForm({ full_name: "", phone: "", email: "", subject: SUBJECTS[0], message: "", preferred_date: "", preferred_time: TIMES[2], modality: "presentiel" }); }}
    ...
    <div style={{ background: "var(--page-bg)", minHeight: "100vh", fontFamily: "var(--font-body)", color: "var(--text-primary)" }}>
      <div style={{ background: "linear-gradient(135deg, #7c3aed20, #7c3aed40)", borderBottom: "1px solid var(--border)", padding: "28px 20px 24px", textAlign: "center" }}>
        ...
        <p style={{ margin: 0, fontSize: 13, color: "var(--text-secondary)" }}>
          Accompagnement confidentiel avec le Rev. Elvis NGUIFFO
        </p>
      </div>
      ...
      <div style={{ maxWidth: 560, margin: "20px auto 0", padding: "0 16px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 20 }}>
          {([
            { key: "presentiel", icon: "🏛️", label: "Presentiel" },
            { key: "visio",      icon: "📹", label: "Visio" },
            { key: "telephone",  icon: "📞", label: "Telephone" },
          ] as const).map((m) => (
            <button key={m.key} onClick={() => setForm({ ...form, modality: m.key })}
              style={{ padding: "14px 10px", background: form.modality === m.key ? "rgba(124,58,237,0.15)" : "var(--card-bg)", border: `1px solid ${form.modality === m.key ? "var(--violet-light)" : "var(--border)"}`, borderRadius: "var(--radius-lg)", cursor: "pointer", textAlign: "center", transition: "all 0.2s" }}>
          ...
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              ...
              <div>
                ...
                <input value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} placeholder="+237 6..." style={inputStyle} required />
              </div>
            </div>
            <div>
              <label style={labelStyle}>Email</label>
              <input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="vous@email.com" style={inputStyle} />
            </div>
            <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <div>
                ...
                <input type="date" value={form.preferred_date} min={new Date().toISOString().split("T")[0]} onChange={(e) => setForm({ ...form, preferred_date: e.target.value })} style={inputStyle} required />
              </div>
              <div>
            ...
            <div>
              ...
              <textarea value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} placeholder="Precisez votre demande si vous le souhaitez..." rows={4} style={{ ...inputStyle, resize: "vertical" }} />
            </div>
            {error && (
              <div style={{ background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.3)", borderRadius: "var(--radius-md)", padding: "10px 14px", color: "var(--error)", fontSize: 12 }}>
                {error}
              </div>
            )}
