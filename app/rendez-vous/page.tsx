...
export default function RendezVousPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!form.full_name.trim() || !form.phone.trim() || !form.preferred_date) {
      setError("Veuillez remplir les champs obligatoires : nom, téléphone, date.");
      return;
    }
    setSending(true);
    setError("");
    ...
    if (!user) { setError("Vous devez être connecté pour prendre rendez-vous."); setSending(false); return; }
      ...
      <div style={{ background: "var(--page-bg)", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: 24, fontFamily: "var(--font-body)" }}>
        <div style={{ textAlign: "center", maxWidth: 400 }}>
          ...
          <p style={{ margin: "0 0 8px", color: "var(--text-secondary)", fontSize: 14, lineHeight: 1.6 }}>
            Merci {form.full_name.split(" ")[0]}. Votre demande de rendez-vous le <strong>{new Date(form.preferred_date).toLocaleDateString("fr-FR")}</strong> à <strong>{form.preferred_time}</strong> a bien été reçue.
          </p>
    ...
    <div style={{ background: "var(--page-bg)", minHeight: "100vh", fontFamily: "var(--font-body)", color: "var(--text-primary)" }}>
      ...
      <div style={{ background: "linear-gradient(135deg, #7c3aed20, #7c3aed40)", borderBottom: "1px solid var(--border)", padding: "28px 20px 24px", textAlign: "center" }}>
        ...
        <p style={{ margin: 0, fontSize: 13, color: "var(--text-secondary)" }}>
          Accompagnement confidentiel avec le Rév. Elvis NGUIFFO
        </p>
      </div>
        ...
        <div style={{ background: "var(--card-bg)", border: "1px solid var(--border)", borderRadius: "var(--radius-xl)", padding: "24px 20px" }}>
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div>
              ...
              <textarea value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} placeholder="Précisez votre demande si vous le souhaitez..." rows={4} style={{ ...inputStyle, resize: "vertical" }} />
            </div>
