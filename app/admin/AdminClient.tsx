...
interface Member {
  ...
  spiritual_level?: string; created_at: string; country?: string; city?: string;
}
interface Post {
  id: string; content: string; created_at: string; user_id: string;
  is_pinned?: boolean; post_type?: string;
}
interface Prayer {
  id: string; title: string; content?: string; is_answered: boolean;
  created_at: string; user_id: string; is_anonymous?: boolean; category?: string;
}
interface Devotion {
  id: string; devotion_date: string; title: string; verse_reference: string;
  verse_text?: string; meditation_p1?: string; meditation_p2?: string;
  meditation_p3?: string; reflection_question?: string; prayer?: string; declaration?: string;
}
interface Event {
...
const sectionTitle: React.CSSProperties = {
  ...
  letterSpacing: "0.08em", color: "var(--text-muted)", margin: "0 0 1rem"
};
...
export default function AdminClient({
  ...
  const deletePost = async (postId: string) => {
    if (!confirm("Supprimer cette publication definitivement ?")) return;
    const sb = createClient();
  ...
  const saveDevotion = async () => {
    ...
    if (error) {
      ...
      setDevMsg({ type: "success", text: "Devotion publiee avec succes !" });
      setForm({
        devotion_date: new Date().toISOString().split("T")[0],
        title: "", verse_reference: "", verse_text: "",
        meditation_p1: "", meditation_p2: "", meditation_p3: "",
      ...
      <div style={{ background: "var(--header-gradient)", borderBottom: "1px solid var(--border)", backdropFilter: "blur(20px)" }}>
        <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "1.25rem 1.5rem 0" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1.25rem", flexWrap: "wrap", gap: "0.75rem" }}>
            <div>
              <p style={{ color: "var(--text-muted)", fontSize: "0.75rem", margin: "0 0 0.2rem", textTransform: "uppercase", letterSpacing: "0.08em" }}>Centre Chretien Berakah</p>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
              ...
              <a href="/dashboard" style={{ color: "var(--text-muted)", fontSize: "0.8rem", textDecoration: "none", padding: "0.4rem 0.9rem", border: "1px solid var(--border)", borderRadius: "9999px", transition: "all 0.2s" }}>
                ← App
              </a>
            </div>
          </div>
      ...
      <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "1.75rem 1.5rem" }}>
        ...
        {tab === "overview" && (
          <div>
            ...
            {/* 2 colonnes : membres récents + dévotions récentes */}
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.5rem" }}>
              <div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                  {members.slice(0, 6).map(m => {
                    ...
                    return (
                      <div key={m.id} style={{ ...card, padding: "0.875rem 1rem", display: "flex", alignItems: "center", gap: "0.75rem" }}>
                        <div style={{ width: "36px", height: "36px", borderRadius: "50%", background: m.role === "admin" ? "var(--gold)" : "var(--violet, #7c3aed)", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: "0.9rem", color: m.role === "admin" ? "#1a0a00" : "white", flexShrink: 0 }}>
                  ...
                  {members.length === 0 && <div style={{ ...card, textAlign: "center", color: "var(--text-muted)", padding: "2rem" }}>Aucun membre.</div>}
                </div>
              </div>
              <div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                  ...
                  {devotions.length === 0 && <div style={{ ...card, textAlign: "center", color: "var(--text-muted)", padding: "2rem" }}>Aucune devotion.</div>}
                </div>
              </div>
            </div>
          </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
              {filteredMembers.map(m => (
                <div key={m.id} style={{ ...card, display: "flex", alignItems: "center", gap: "1rem", flexWrap: "wrap" }}>
                  <div style={{ width: "42px", height: "42px", borderRadius: "50%", background: m.role === "admin" ? "var(--gold)" : "var(--violet, #7c3aed)", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: "1rem", color: m.role === "admin" ? "#1a0a00" : "white", flexShrink: 0 }}>
                    {(m.full_name?.[0] ?? "?").toUpperCase()}
                  </div>
                  <div style={{ flex: 1, minWidth: "150px" }}>
                    <div style={{ fontWeight: 700, fontSize: "0.92rem" }}>{m.full_name || "Sans nom"}</div>
                    <div style={{ fontSize: "0.75rem", color: "var(--text-muted)" }}>
                      {m.country || m.city || "Localisation inconnue"} · {m.spiritual_level || "Nouveau croyant"}
                    </div>
                  </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
              {posts.map(p => (
                <div key={p.id} style={{ ...card, display: "flex", gap: "1rem", alignItems: "flex-start" }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.4rem", flexWrap: "wrap" }}>
                      ...
                      {p.post_type && p.post_type !== "text" && (
                        <span style={{ fontSize: "0.68rem", padding: "0.15rem 0.5rem", borderRadius: "9999px", background: "var(--surface)", color: "var(--text-muted)", border: "1px solid var(--border)" }}>{p.post_type}</span>
                      )}
                      {p.is_pinned && (
                        <span style={{ fontSize: "0.68rem", padding: "0.15rem 0.5rem", borderRadius: "9999px", background: "rgba(212,175,55,0.12)", color: "var(--gold)" }}>📌 Epingle</span>
                      )}
                  ...
                  <button onClick={() => deletePost(p.id)} style={{ padding: "0.4rem 0.75rem", borderRadius: "var(--radius-md)", border: "1px solid rgba(239,68,68,0.3)", background: "rgba(239,68,68,0.08)", color: "#f87171", fontSize: "0.78rem", cursor: "pointer", flexShrink: 0, whiteSpace: "nowrap" }}>
                    🗑 Supprimer
                  </button>
                </div>
              ))}
              {posts.length === 0 && (
                <div style={{ ...card, textAlign: "center", color: "var(--text-muted)", padding: "3rem" }}>Aucune publication.</div>
              )}
            </div>
          </div>
        )}
        ...
        {tab === "prayers" && (
          <div>
            <p style={{ ...sectionTitle, marginBottom: "1rem" }}>
              {prayers.filter(p => !p.is_answered).length} priere(s) ouverte(s) · {prayers.filter(p => p.is_answered).length} repondue(s)
            </p>
            <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
              {prayers.map(p => (
                  ...
                  <div style={{ display: "flex", alignItems: "flex-start", gap: "1rem" }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.35rem", flexWrap: "wrap" }}>
                        ...
                        <span style={{ fontSize: "0.68rem", padding: "0.15rem 0.5rem", borderRadius: "9999px", background: p.is_answered ? "rgba(212,175,55,0.12)" : "rgba(244,114,182,0.12)", color: p.is_answered ? "var(--gold)" : "#f472b6" }}>
                          {p.is_answered ? "✓ Repondue" : "● Ouverte"}
                        </span>
                      ...
                      {p.content && (
                        <div style={{ fontSize: "0.85rem", color: "var(--text-secondary)", overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" }}>{p.content}</div>
                      )}
                    </div>
                    {!p.is_answered && (
                      <button onClick={() => closePrayer(p.id)} style={{ padding: "0.4rem 0.85rem", borderRadius: "var(--radius-md)", border: "1px solid rgba(212,175,55,0.3)", background: "rgba(212,175,55,0.08)", color: "var(--gold)", fontSize: "0.78rem", cursor: "pointer", flexShrink: 0, whiteSpace: "nowrap" }}>
                        ✓ Repondue
                      </button>
                    )}
                  </div>
              ...
              {prayers.length === 0 && (
                <div style={{ ...card, textAlign: "center", color: "var(--text-muted)", padding: "3rem" }}>Aucune priere.</div>
              )}
            </div>
          </div>
        )}
        ...
        {tab === "devotions" && (
          <div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: "1rem" }}>
                    ...
                    <div>
                      ...
                      <input type="text" placeholder="Car Dieu a tant aime le monde..." value={form.verse_text} onChange={e => setForm(f => ({ ...f, verse_text: e.target.value }))} style={inputStyle} />
                    </div>
                  </div>
                  <div>
                    <label style={labelStyle}>Meditation 1 *</label>
                    <textarea placeholder="Premier paragraphe de meditation..." value={form.meditation_p1} onChange={e => setForm(f => ({ ...f, meditation_p1: e.target.value }))} style={{ ...inputStyle, minHeight: "90px", resize: "vertical" }} />
                  </div>
                  <div>
                    <label style={labelStyle}>Meditation 2 (optionnel)</label>
                    <textarea placeholder="Deuxieme paragraphe..." value={form.meditation_p2} onChange={e => setForm(f => ({ ...f, meditation_p2: e.target.value }))} style={{ ...inputStyle, minHeight: "90px", resize: "vertical" }} />
                  </div>
                  <div>
                    <label style={labelStyle}>Meditation 3 (optionnel)</label>
                    <textarea placeholder="Troisieme paragraphe..." value={form.meditation_p3} onChange={e => setForm(f => ({ ...f, meditation_p3: e.target.value }))} style={{ ...inputStyle, minHeight: "90px", resize: "vertical" }} />
                  </div>
                  <div>
                    <label style={labelStyle}>Question de reflexion (optionnel)</label>
                    <input type="text" placeholder="Comment appliquer ce verset aujourd hui ?" value={form.reflection_question} onChange={e => setForm(f => ({ ...f, reflection_question: e.target.value }))} style={inputStyle} />
                  </div>
                  <div>
                    <label style={labelStyle}>Priere *</label>
                    <textarea placeholder="Seigneur, je te prie..." value={form.prayer} onChange={e => setForm(f => ({ ...f, prayer: e.target.value }))} style={{ ...inputStyle, minHeight: "90px", resize: "vertical" }} />
                  </div>
                  <div>
                    <label style={labelStyle}>Declaration de foi *</label>
                    <input type="text" placeholder="Je declare que..." value={form.declaration} onChange={e => setForm(f => ({ ...f, declaration: e.target.value }))} style={inputStyle} />
                  </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
              ...
              {devotions.length === 0 && (
                <div style={{ ...card, textAlign: "center", color: "var(--text-muted)", padding: "3rem" }}>
                  Aucune devotion. Creez la premiere en cliquant sur &quot;+ Nouvelle devotion&quot; !
                </div>
              )}
            </div>
          </div>
