...
export default function ContactPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error: e2 } = await supabase.from("contact_messages").insert({
    ...
    return (
      <div style={{ background: "var(--page-bg)", display: "flex", alignItems: "center", justifyContent: "center", padding: "24px 16px", fontFamily: "var(--font-body)" }}>
        <div style={{ textAlign: "center", maxWidth: 400 }}>
          <div style={{ width: 80, height: 80, borderRadius: "50%", background: "rgba(5,150,105,0.15)", border: "2px solid #059669", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 20px", fontSize: 36 }}>
            ✅
          </div>
    ...
    <div style={{ background: "var(--page-bg)", fontFamily: "var(--font-body)", color: "var(--text-primary)" }}>
      <div style={{ background: "linear-gradient(135deg, #05966920, #05966940)", borderBottom: "1px solid var(--border)", padding: "28px 20px 24px", textAlign: "center" }}>
        ...
        <p style={{ margin: 0, fontSize: 13, color: "var(--text-secondary)" }}>
          {"L'équipe CCB répond généralement en 24–48h"}
        </p>
      </div>
      ...
      <div style={{ display: "flex", gap: 0, borderBottom: "1px solid var(--border)", overflowX: "auto" }}>
        {[
          ...
          { icon: "📞", label: "+237 XXX XXX XXX" },
          { icon: "📧", label: "contact@ccb.cm" },
        ].map((item) => (
      ...
      <div style={{ maxWidth: 560, margin: "0 auto", padding: "24px 16px 32px" }}>
        <div style={{ background: "var(--card-bg)", border: "1px solid var(--border)", borderRadius: "var(--radius-xl)", padding: "24px 20px" }}>
          <h2 style={{ margin: "0 0 20px", fontSize: 16, fontWeight: 700, color: "var(--text-primary)", fontFamily: "var(--font-title)" }}>
        ...
        <div style={{ marginTop: 20, background: "var(--card-bg)", border: "1px solid var(--border)", borderRadius: "var(--radius-xl)", padding: "20px" }}>
          <h3 style={{ margin: "0 0 14px", fontSize: 14, fontWeight: 700, color: "var(--text-primary)", fontFamily: "var(--font-title)" }}>
            🕐 Horaires des cultes
          </h3>
          {[
